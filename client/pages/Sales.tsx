...
export default function Sales() {
  ...
  const parseOrderDate = (dateStr: string): Date | null => {
    ...
    try {
      ...
      if (dateStr.includes('T')) {
        ...
        if (parts[0].length === 4) {
          // YYYY-MM-DD
          return new Date(`${datePart} ${timePart || '00:00:00'}`);
        } else {
          // DD-MM-YYYY
          const [day, month, year] = parts;
          return new Date(`${year}-${month}-${day} ${timePart || '00:00:00'}`);
        }
      } else {
        return new Date(dateStr);
      }
  ...
  // Filter orders by period, search, status, and region
  const filteredOrders = orders.filter((order) => {
    // Filter by period
    const orderDate = parseOrderDate(order.creation_date);
    if (orderDate) {
      const periodRange = getPeriodRange(period as "7d" | "30d" | "90d" | "ytd");
    ...
    // Filter by search, status, and region
