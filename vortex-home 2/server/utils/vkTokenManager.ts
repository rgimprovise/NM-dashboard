...
export function saveTokens(tokens: TokenData): void {
  try {
    // Check if fs is available (may not be in all environments like Builder.io)
    if (typeof fs === 'undefined' || !fs.existsSync || !fs.writeFileSync) {
      console.warn('⚠️ File system not available, skipping token save');
      // Still update process.env and cache
...
export function initTokenManager(): void {
  try {
    ...
    if (typeof process === 'undefined' || !process.env) {
      console.warn('⚠️ VK Token Manager: Not in Node.js environment, skipping initialization');
      return;
    }
  ...
  if (tokens) {
    ...
    // Continue without token manager - API calls will handle missing tokens
  }
}
